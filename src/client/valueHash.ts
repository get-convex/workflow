...
/** Runtime-neutral SHA-256 over Convex's canonical JSON representation. */
export function hashConvexValue(value: Value): string {
  const bytes = new TextEncoder().encode(JSON.stringify(convexToJson(value)));
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let i = 0; i < 16; i++) {
      words[i] = data.getUint32(offset + i * 4);
    }
    for (let i = 16; i < 64; i++) {
      const x = words[i - 15];
      const y = words[i - 2];
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const temp1 =
        (h + sum1 + choose + SHA256_ROUND_CONSTANTS[i] + words[i]) >>> 0;
